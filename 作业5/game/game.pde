...
void draw() {
  ...
  if(t==0) {
    ...
    strokeWeight(10);
    line(pos-barsize,heightbounce,pos+barsize,heightbounce);
    strokeWeight(10);
    ...
    if (y>=heightbounce && y<heightbounce+10) {
      ...
      if (x>(pos-barsize) && x< (pos-(1./3)*barsize) && dx>0 && dy>0) {
        dy*=-0.95;
        dx*=-1.05;
      }
      //left part - right left - get more horisontal
      else if (x>(pos-barsize) && x< (pos-(1./3)*barsize) && dx<0 && dy>0) {
        dy*=-0.95;
        dx*=1.05;
      }
      //right part - left right - get more horisontal
      else if (x>(pos+(1./3)*barsize) && x< (pos+barsize) && dx>0 && dy>0) {
        dy*=-0.95;
        dx*=1.05;
      }
      ...
      else if (x>(pos-(1./3)*barsize) && x< (pos+(1./3)*barsize) && dy>0) {
        dy*=-1.05;
        dx*=0.95;
      }
    }
